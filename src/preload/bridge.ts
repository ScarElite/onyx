...
/**
 * Expose the Onyx bridge on `window.onyx`.
 *
 * A function rather than a side effect so V's Hub can call it from ITS preload —
 * the Hub has its own `window.vbridge` to expose alongside, and it must not
 * hardcode Onyx's channel names (they would drift from `CH` the first time one
 * changed). One definition, both preloads.
 */
export function exposeOnyxBridge(): void {
